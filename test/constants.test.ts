...
const OUTSIDE: readonly Group[] = [
...

  {
    why: 'STAYS: the drifting-cargo field\'s own furniture, none of it a tunable rule.'
      + ' `CANISTER_HULL`, `POD_HULL` and `DRONE_HULL` are memoised `requireShipDef` lookups, `LOOK`'
    ...
    files: {
      'game/cargo.ts': ['CANISTER_HULL', 'POD_HULL', 'DRONE_HULL', 'LOOK', 'POLICY', 'SPIN_RATE'],
    },
  },
...

  {
    ...
    files: {
      ...
      // ...and the two lists of buttons a thumb presses in flight (docs/TODO/204)
      'ui/key-help.ts': ['LABELS', 'ALL_BINDINGS', 'STATION_MENU_NOTE', 'TOUCH_COMMANDS', 'TOUCH_MENU'],
...

  {
    why: 'STAYS: player-facing WORDS, and a derivation rather than a value —'
      + ' `DISREPUTE_BRIBE > 0 ? " AND YOUR REPUTATION" : ""`. The rule it reports lives in'
      ...
      + ' claim automatically',
    files: {
      'game/prompts.ts': ['REPUTATION_COST'],
    },
  },
];
