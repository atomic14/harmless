...
// The game has two kinds: a signed contract, and a live mission. GitHub #27
...
// REAL mission machine through `runMissions`, because a fixture that sets a
// leg by hand would pass while the machine that sets it was broken.
...
import { standingOrders, ordersSummary, type MissionOrder } from '../src/game/orders.ts';
...
console.log('\nstanding orders — every kind the commander holds is named');
{
  ...
  const cmdr = (over: Partial<CommanderData> = {}): CommanderData => ({
    ...newCommander(),
    kills: 0, galaxy: 1, systemIndex: 7, day: 100, contracts: [], ...over,
  });
  ...
  const half = () => 0.5;
  const accept = (c: CommanderData): void => {
    runMissions(c, { kind: 'accept', skeleton: 'constrictor' }, systems, half);
  };
  ...
  // 1. the hunt ------------------------------------------------------------
  {
    ...
    accept(c);
    eq('the machine opens the hunt', c.missions.live[0]?.leg, 'hunt');
    ...
    check('...and it is a mission that gave it', orders[0]?.kind === 'mission');
  }

  // 2. the report, and the courier run ---------------------------------------
  {
    const c = cmdr({ missions: constrictorAt('report', null) });
    const between = standingOrders(c, systems);
    eq('between the two legs the order is to report, at any station', between.length, 1);
  ...
  // line printed the first contract and stopped.
  {
    ...
    eq('...the mission sorts above the work, because it is briefed one time',
      orders[0]?.kind, 'mission');
...
//
// THE MISSIONS SCREEN CARRIES THE PATRONS' ORDERS ONLY (docs/TODO/145). Board
// work has its own screen; test/contracts-screen.test.ts holds that half.
...
console.log('\nthe MISSIONS screen draws the held legs, and nothing else');
{
  ...
  const paint = (c: CommanderData): string =>
    captureById(() => {
      renderMissions({
        offers: [], held: held(c, systems).map((o) => ({ ...o, patron: 'THE NAVY', choices: [], title: 'NAVY MISSION', pages: [] })),
  ...
  c.equipment.laser = 'beam';
  runMissions(c, { kind: 'accept', skeleton: 'constrictor' }, systems, () => 0.5);
...
console.log('\nno leg of the mission has a target the orders do not carry');
{
  const systems = generateGalaxy(1);
  const c: CommanderData = {
    ...newCommander(), kills: 16, galaxy: 1, systemIndex: 7, day: 100, contracts: [],
  };
  const half = () => 0.5;

  // Walk the machine end to end rather than set a leg by hand. A leg added
  // later walks through here too, which a fixture per leg would not.
  runMissions(c, { kind: 'accept', skeleton: 'constrictor' }, systems, half);
  const seen: string[] = [];
  let carried = 0;
  for (let turn = 0; turn < 6 && c.missions.live.length > 0; turn++) {
    const live = c.missions.live[0];
    seen.push(live.leg);
    const target = live.target;
    if (target !== null) {
      const named = systems[target].name.toUpperCase();
      check(`the ${live.leg} leg names ${named} in the standing orders`,
        standingOrders(c, systems).some((o) => o.line.includes(named)));
      carried += 1;
      c.systemIndex = target;
    }
    // fly there, then take the leg off the board. The kill is the one event
    // the machine cannot reach on its own.
    if (live.tag !== null) runMissions(c, { kind: 'destroyed', tag: live.tag }, systems, half);
    else runMissions(c, { kind: 'docked' }, systems, half);
  }
  check(`the walk reached every leg that has a target (${seen.join(' → ')})`, carried >= 2);
  eq('...and the machine ran out at completion', c.missions.done.constrictor, 'complete');
...
console.log('\nthe summary never drops a kind for another');
{
  ...
  const heldBy = (navy: boolean, jobs: number): CommanderData => {
    const c: CommanderData = { ...base, contracts: [], missions: structuredClone(base.missions) };
    if (navy) runMissions(c, { kind: 'accept', skeleton: 'constrictor' }, systems, () => 0.5);
