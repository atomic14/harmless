...
// over which it may spend anything at all, as a fade rather than a cap. The axis
// the turn steers its roll against goes degenerate as the nose arrives
...
//   a write to `player.speed`. A demand has a THROTTLE, which is full ahead,
//   full astern or coast. So the hull's own thrust holds the plan's speed, with
//   a deadband of one frame's worth of it.

/**
 * How close to the station the docking computer will take the job, in world
 * units.
 *
 * It is far enough to be useful from the end of the torus run. It is near enough
 * to be an approach aid rather than a taxi across the system. It equals the
 * player's `LASER_RANGE` by a coincidence of scale, and not by rule.
 */
 ...
 * `DC_TURN_FADE_ANGLE` or more. So the turn holds its full claim right up to
 * the mouth, and banks by exactly this much. Undamped, the ship swung through
 * ±40
 ...
 * this is the answer.
 *
 * `npm run dock-probe` flies 504 approaches at a lead of 0.10. The median
 * approach goes through the slot 3.1, 3.8, 4.3, 5.0 and 5.5 degrees off its
 * long axis. Those five readings are at a margin of 0.25, 0.28, 0.30, 0.33 and
 * 0.35. That is a
 * straight line through this constant, which is the mechanism in plain sight.
 *
 * SO IT IS CHOSEN AT THE KNEE, RATHER THAN AS LOW AS IT WILL GO. Below 0.30 the
 * median keeps falling, and the WORST case blows out past where it started. It
 * is 33.5 degrees at 0.25 and 34.3 at 0.28, against 30.0 before this item. That
 * happens
 ...
 * `nose × heading` (`dockingSticks`). That is the axis the wings must lie on for
 * a pull to shorten the heading error.
 *
 * That vector's LENGTH is `sin` of this very angle, so it vanishes exactly when
 * the controller succeeds. A vector of no length still has a direction, and that
 * direction is then numerical residue.
 *
 * Ungated, the autopilot flew four seconds dead on the gate heading. It rolled
 * hard over and back every 0.45s at 1.3 rad/s, in a chase after that residue
 * (docs/TODO/134, GitHub #23).
 *
 ...
 * needs, and the reversals climb back to 15. The ship holds the correction back
 * until the error is large, and then corrects hard.
 *
 ...
 * other. They are one unlucky approach apiece, and not a signal.
 *
 * The median agrees on both. It is 8 and 9 here, and 20 and 9 before and after
 * on the second grid. That is what chose it.
 *
 ...
 * suppresses an axis that is genuinely MEANINGLESS near zero. It must give the
 * turn its authority back as soon as there is a real turn to make. They have
 ...
 * that axis. Here the axis has a second job, which is the fit through the slot.
 * So the claim goes all the way to zero, and the slot's own roll stands.
 *
 ...
 * about 7 per second. The ramp time constant is a quarter of a second. That
 ...
 * disturbance never stops. The ship rolls past the bank, and its pitch plane
 * sweeps with it. The heading error then moves in its own frame, and the roll
 * is asked for again. Held on a fixture with the demand still, a 30-degree bank
 * overshoots by 7.6 degrees, reverses twice, and takes 1.5s to settle. With
 * this term it overshoots by 1.8, reverses once, and settles in 1.0s. A
 * 10-degree bank does not reverse at all (`test/docking-computer.test.ts`).
 *
 ...
 * be in a tenth of a second? Ask for THAT. The arithmetic behind the value is
 * the same. This, times the gain, is the damping the loop was missing. 0.23s
 ...
 * monotonically with it. It is 18 with no term at all, 14 at 0.06, 12 at 0.10,
 * 10 at 0.16, and 9 at 0.28. So does the roll swept, from 1.9 turns to 0.9. But
 * past
 ...
 * error. It is 1.8 degrees, and this term adds to it: 3.3 at 0.10, and 4.8 at
 * 0.20.
 *
 * To credit the roll that the slot itself asks for removes the added lag
 * exactly, and on the fixture it does. The standing error stays 1.8 at any lead.
 *
 * In flight it is worth 0.1 degrees at the letterbox, on both grids. What the
 * wings arrive with is set by the bank the turn holds (`DC_SLOT_MARGIN`), and
 * not by the lag. It cost a field on the plan, an import of the station's spin into the
 ...
 * from the ship to the slot. The ship flies at the point one of these along it,
 * from wherever it is on it.
 *
 * Everything the path buys is bought here. The aim is never a point the ship
 * sits on, so the heading to it is well conditioned. It always moves forward
 * along the curve, so the heading cannot reverse. It also rounds the curve's
 * corners for free, because an aim point that rounds a corner moves
 ...
 * letterbox asks about.
 *
 * Short, and the ship tracks the curve tightly. It arrives pointing straight but
 * rolled. At one half-width, the median approach goes through 3.4 degrees off
 * the axis and 16.4 off the slot.
 *
 * Long, and it flies inside the curve, by roughly the square of this over twice
 * the radius. It therefore arrives with more to correct, and a correction is an
 * angle. At two half-widths it is 7.9 off the axis and 12.1 off the slot, with
 * 60 scrapes, which is the cliff.
 *
 * One and a half is between them and clear of it: 5.4 and 7.5, with nothing
 * that scrapes.
 *
 * IT DOES NOT VARY. Two rules that made it vary were tried and measured away.
 *
 * A clamp to a share of what is LEFT of the path reads well. It is what makes
 * the last hundred units twitchy. A ship 7 units off a 26-wide channel was asked
 * for an 11-degree correction, because the aim closed to 84 units.
 *
 * An extension to the path's END, once the ship is on the straight leg, gives
 * the old approach's serene 2.9-degree entry. It also steps the aim 200 units
 * when the projection crosses onto that leg, and the median jump goes from 0.6
 * degrees to 13.9.
 *
 * A flat lookahead has neither problem, and it needs no case analysis.
 *
 ...
 *
 * It replaced a constant of the same name that meant something else. That one
 * was how far down the axis the old gate aim was allowed to LEAD, once the ship
 * earned it by being lined up.
 *
 * That was docs/TODO/135's fix to a plan that jumped when the run committed, and
 * it is subsumed. With a path there is no commit to jump at, because the run in
 * is the last leg of the same curve.
 *
