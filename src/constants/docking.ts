// How to thread the station slot. Three things: the letterbox, the approach
// that lines you up on it, and the cube that says you hit the hull instead.
//
...
//
// Both released stations put their slot on the front face. It is a rectangle
// TALLER THAN IT IS WIDE in station-local coordinates: 20x60 at a Coriolis, and
// 32x64 at a Dodo. So a ship's wings line up with the station's local Y.
 ...
 * ship that comes from behind flies round at the gate radius. Everything it has
 * to do to get in then happens over this much bearing.
 *
 ...
 * sharper turn, flown closer in. The ship then starts to cut the corner of the
 * box, rather than the curve.
 */
export const TURN_IN = Math.PI / 4;

/**
 * Where the approach stops the curve and flies straight, in station half-widths.
 * It is the radius at which `dock-path.ts`'s funnel meets the slot axis. So it
 * is also the length of the RUN IN that follows. It is three fifths of the
 * gate distance, which is three half-widths at both released stations.
 *
 * A ship should point down the slot before it is in the slot. With no straight
 * leg, the funnel dives all the way to the letterbox, and the ship goes through
 * still on the turn. That measured 13.6 degrees off the axis in a median
 * approach, and 19.3 at worst.
 *
 * A pilot feels that as a slot that arrives too fast, and at an angle (Chris,
 * flying docs/TODO/136: *"it feels quite tight into the slot so the angle seems
 * a bit too much"*). With the leg it is 5.4 and 12.4, against 2.9 and 7.3 for
 * the branch-and-corridor approach that both replaced.
 *
 ...
 * `dockingOutcome`'s box is inside this radius. So the curve never enters the
 * box at all, and the straight leg enters it dead on the axis.
 *
 * It is bounded ABOVE by the gate, because the funnel needs radius to spend on
 * its dive. At four fifths the dive is squeezed into a corner the follower has
 * to round. The plan's own heading then moves 12 degrees in a frame where three
 * fifths moves 0.7.
 *
 * It is tied to `GATE_HALF_WIDTHS` rather than stated flat, so that a bigger
 * gate keeps the same shape of funnel.
 *
 ...
 * error into the hull rather than into the slot. Once committed, `planDocking`
 * tolerates twice this.
 *
 * The phase latches. Without that latch, a fall in `along` flips it back to
 * 'gate', and the ship oscillates and never docks. The same figure is the
 * lateral half of `arrived`, which is what an NPC trader docks on.
 */
export const LINED_UP_LATERAL = 45;

/**
 * The bounding cube around the station: a margin over the half-width, in world
 * units, a little larger than the hull. Inside it, you are in the slot channel,
 * or you struck the hull.
 *
 ...
 *  traffic bounces off a Dodo's hull where a smaller margin let a ship clip
 *  it. */
