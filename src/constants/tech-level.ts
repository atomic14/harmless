// The tech level scale: the shown range every system's byte can encode.
//
// galaxy/galaxy.ts computes a raw zero-based `techLevel` of 0-14, and every
// reader adds one before showing it, which turns it into the shown 1-15. The
// ceiling is a literal with its derivation written out rather than a read of the
// algorithm, because this directory may not import galaxy.ts.

/** The lowest tech level any system shows. */
export const TECH_MIN = 1;

/** The highest — the algorithm's own ceiling of 14, shown as 15. */
export const TECH_MAX = 15;
