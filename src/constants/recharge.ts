...
// source gives each hull only an `energyRechargeRating` and no clock, so what a
// rating is worth in seconds is Harmless policy, and this file states it.
//
 ...
 * no build the galaxy sends may be one that a face simply outruns. The bound is
 * the lightest gun in the roster — the Worm and the Ophidian, at 3.27 points a
 ...
 * the pool that keeps her alive. A cut to it compounds a lethality that the item
 * did not ask for, and 40 seconds is the figure a Cobra flew before the pools
 * grew.
 */
