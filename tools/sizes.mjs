...
const ALLOWED = {
  ...
  'game/game.ts': 'DEBT: the orchestrator, and 1,810 lines of it. It stays a DEBT rather than an ok while the decision below is open. 3,244 -> 2,528 -> 2,251 -> 2,153 -> 2,021, then 2,049 where docs/TODO/151 corrected twenty-one comments that named a caller the file no longer has, then 1,910 and 1,810. docs/TODO/150 took five responsibilities out, each beside the rules it spends: the law to game/law-actions.ts, the sky to game/world-build.ts, the cockpit to game/cockpit-view.ts, the jump to game/hyperspace-actions.ts and the career to game/career.ts. THE OLD TARGET OF ~300 IS NOT REACHABLE BY TAKING MORE AREAS OUT, and 150 M6 is the measurement that says so rather than an opinion: 75% of what is left cannot leave — the appliers (an applier IS the orchestrator, by this project\'s own decides/applies rule), the six host literals, the 81-line command table that is deliberately the whole driving surface, the frame, the input and the constructor that wires the five children. Only 448 of the 1,810 lines sit in members that could still move, so taking all 44 out lands the file near 1,362 — a real shrink of about a quarter, and then it stops. What rose is the price rather than a fall in the shrink: game.ts has come down a steady 98 to 138 lines a milestone since M1, while the lines WRITTEN per line REMOVED went 1.09, 1.90, 1.87, 1.92, 2.61. So this entry states a floor rather than a target, and the next move is a scope decision for Chris, recorded in the plan: re-aim the target here, keep taking marginal areas at about 150 lines of tree per 90 removed, or split the ORCHESTRATOR — a docked half and a flight half — which is the only path that reaches ~300. The number above moves with each milestone, so the entry cannot go stale while the work is live.',
