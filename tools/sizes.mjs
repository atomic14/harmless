...
const ALLOWED = {
  ...
  'game/game.ts': 'DEBT: the orchestrator, and 1,222 lines of it. IT CAN STATE ONE RESPONSIBILITY NOW, which is the bar this list actually sets: which mode the game is in, and who gets the frame. It owns baseMode, hands each frame to the half that owns it, routes every key to the child that answers it, and says what those children report. docs/TODO/155 split the two modes apart: M1 took the docked half to game/docked.ts, and M2 the flight half to game/flight.ts, which is itself a parent over game/flight-weapons.ts and game/flight-instruments.ts. THERE IS NO LINE TARGET HERE ANY MORE, and that is a decision rather than a surrender (Chris, 2026-08-14: "we should not obsess over the 300 lines. What we are looking for is a clean architecture"). The "~300" this entry carried for months was a number nobody had checked, and docs/TODO/150 M6 checked it: 75% of what is left cannot leave, because an applier IS the orchestrator by this project\'s own decides/applies rule, the six host literals travel with the modules they serve, the 81-line command table is deliberately the whole surface a replay, an AI or a test drives the game through, and the constructor is where the children are wired. Only 448 of the 1,810 lines sit in members that could still move. So the question this entry now asks is the file header\'s question and not a count: CAN THIS FILE STATE ONE RESPONSIBILITY? Today it is the mode machine for two modes at once, which is why docs/TODO/155 is splitting docked from flight — Chris again: "they are very different things. Why would you want to couple them together?" The history, because it is useful: 3,244 -> 2,528 -> 2,251 -> 2,153 -> 2,021, then 2,049 where docs/TODO/151 corrected twenty-one comments that named a caller the file no longer has, then 1,910 and 1,810. docs/TODO/150 took five responsibilities out, each beside the rules it spends — the law to game/law-actions.ts, the sky to game/world-build.ts, the cockpit to game/cockpit-view.ts, the jump to game/hyperspace-actions.ts, the career to game/career.ts — at a price that rose from 1.09 to 2.61 lines written per line removed. It stays a DEBT while 155 is open.',
