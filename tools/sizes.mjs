...
const ALLOWED = {
  ...
  'game/controls.ts': "the tables AND the scan that reads them. Its three load-bearing rules are each a property of the PAIR — first-match-wins is why a table's order is data, `independent` is why the view keys run before the chain, and shift-tested-before-consume is why a shifted entry sits above its plain twin — so splitting the matcher out would put each rule in a different file from the entries it governs. It crossed 400 on docs/TODO/146, which gave a tap its own shift so a click can press \u21e7T. Three items in a row (144, 145, 146) each added a command and each had to cut prose to stay under, which is the signal that the ceiling had stopped measuring anything here rather than that the file had stopped being one thing. THE SPLIT IT IS WAITING FOR is the `Command` union itself, a vocabulary of 40 intents that `command-help.ts` is already welded to by `Record<Command, \u2026>`; it is not taken yet because the common change is ADDING a command, and that would go from touching two files to touching three.",
