...
const ALLOWED = {
  ...
  'game/npc.ts': "DEBT: one ship's decision loop, plus three subjects that are not it. THIS ROW USED TO NAME ONE SPLIT — 'behaviour and brain flight in one file; the flight half wants its own' — and docs/TODO/169 measured the file rather than trusting it. There are four candidates, and the flight half is the SMALLEST of them at 244 lines of the class. Taking it alone leaves about 1,399 lines behind.\n\nTHE CHEAPEST CUT IS THE FLEET QUERIES, which this row never named: isHostileToPlayer, engaging, hostilesNear, nearestEngaging and nearestNpc, at 101 lines of pure function over a fleet. isHostileToPlayer is a law rule that six surfaces read (docs/TODO/158), and it sits inside a class file today. The other two candidates are NpcState at 184 lines, which is the saved shape of a ship rather than a behaviour, and the shared flight maths — steerQuatToward and velocityOf — at 29 lines, which the HUD's lead marker reads.\n\nTHE FILE STATES ITS OWN RESPONSIBILITY NOW. It was the one file of 259 in src/ with no module header at all, and docs/TODO/169 M1 wrote one before any split, so the header could not be written to fit a decision already made. M4 of that item measures again and reports what is left.",
