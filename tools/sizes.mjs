...
const ALLOWED = {
  ...
  'game/game.ts': 'DEBT: 3,244 -> 2,528 -> 2,251 -> 2,153 -> 2,021, then back to 2,049 where docs/TODO/151 M1 corrected twenty-one comments that named a caller the file no longer has, and on to 1,910 and 1,810. Still the orchestrator plus leftovers. Target ~300. docs/TODO/150 is decomposing it a responsibility at a time, into children that sit beside the rules they spend — M1 took the law to game/law-actions.ts, M2 the sky to game/world-build.ts, M3 the cockpit to game/cockpit-view.ts, M4 the jump to game/hyperspace-actions.ts and M5 the career to game/career.ts. The number here moves with each milestone, so the entry cannot go stale while the work is live.',
